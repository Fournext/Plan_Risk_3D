...
/*
  async onSubmit(): Promise<void> {
    ...
    try {
      // 🔹 Extraer datos de la tarjeta del formulario
      ...
      const [exp_month, exp_year] = expiryDate.split('/').map((v: string) => v.trim());

      // 🔹 Crear token con Stripe
      const tokenResult = await this.stripe.createToken('card', {
        number: cardNumber,
        exp_month: parseInt(exp_month),
        exp_year: parseInt(`20${exp_year}`), // convierte "25" -> 2025
        cvc: cvv,
        name: cardholderName
      });

      if (tokenResult.error) {
        this.toastr.error(tokenResult.error.message || 'Error generando token', 'Error');
        this.isProcessing = false;
        return;
      }
      ...
      // 🔹 Enviar al backend solo el token, no los datos sensibles
      const paymentData = {
        plan: this.selectedPlan.id,
        monto: this.selectedPlan.price,
        token: token
