...
def limpiar_respuesta_gemini(texto):
    """
    Limpia la respuesta de Gemini removiendo bloques de código markdown y espacios extra
    
    Args:
        texto: Respuesta cruda de Gemini (string con JSON)
    
    Returns:
        dict: JSON parseado como objeto Python (no string con escapes)
    """
    import re
    ...
    try:
        # Parsear el JSON a objeto Python (esto elimina los \n automáticamente)
...
def extraer_info_glb(archivo_glb):
    """
    Extrae información estructural relevante de un archivo .glb:
    ...
    try:
        ...
        # 1️⃣ EXTRAER IMÁGENES EMBEBIDAS (convertir a base64)
        if gltf.images:
            for idx, image in enumerate(gltf.images):
                try:
                    ...
                    # Imagen con URI externa (poco común en exports de Three.js)
                    elif hasattr(image, 'uri') and image.uri and image.uri.startswith('http'):
                        texturas_extraidas[idx] = {
                            'url': image.uri,
                            'mime_type': 'image/png'
                        }
        ...
        # 2️⃣ MAPEAR MATERIALES Y SUS TEXTURAS
        if gltf.materials:
            for idx, material in enumerate(gltf.materials):
                nombre_material = material.name if material.name else 'Material_{}'.format(idx)
                ...
                if material.pbrMetallicRoughness:
                    ...
                    mat_info['metalico'] = pbr.metallicFactor if hasattr(pbr, 'metallicFactor') else 0
                    mat_info['rugosidad'] = pbr.roughnessFactor if hasattr(pbr, 'roughnessFactor') else 0.5
                    ...
                    if hasattr(pbr, 'baseColorTexture') and pbr.baseColorTexture is not None:
                        ...
                        if hasattr(texture_info, 'index') and texture_info.index is not None:
                            ...
                            # Obtener la imagen asociada a esta textura
        ...
        # 3️⃣ AGREGAR TEXTURAS AL RESULTADO
        ...
        if gltf.nodes and gltf.meshes:
            for node_idx, node in enumerate(gltf.nodes):
                ...
                if hasattr(node, 'mesh') and node.mesh is not None:
                    ...
                    nombre_elemento = node.name if node.name else mesh.name if mesh.name else 'Elemento_{}'.format(node_idx)
                    ...
                    if mesh.primitives and len(mesh.primitives) > 0:
                        ...
                        if hasattr(prim, 'material') and prim.material is not None:
                            material_info = materiales_map.get(prim.material, {})
                            material_nombre = material_info.get('nombre', 'sin_material')
                            tipo_material = material_info.get('tipo_material', 'desconocido')
                    ...
                    elemento = {
                        ...
                        'textura_idx': textura_idx,  # Índice de textura que usa
                        'posicion': posicion if posicion else {'x': 0, 'y': 0, 'z': 0}
                    }
        ...
        if 'tmp_path' in locals() and os.path.exists(tmp_path):
            os.unlink(tmp_path)
        raise Exception('Error al extraer información del GLB: {}'.format(str(e)))


def call_gemini_with_images(parts):
    """
    Llama a la API de Gemini con texto e imágenes (multimodal)
    
    Args:
        parts: Lista de partes que pueden ser texto o imágenes inline_data
               Ejemplo: [{"text": "..."}, {"inline_data": {"mime_type": "image/png", "data": "base64..."}}]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'candidates' in result and len(result['candidates']) > 0:
                analisis_texto = result['candidates'][0]['content']['parts'][0]['text']
                return {
                    'success': True,
                    'data': analisis_texto
                }
            ...
            return {
                'success': False,
                'error': 'Error de la API de Gemini: {} - {}'.format(response.status_code, response.text)
            }
    
    except Exception as e:
        return {
            'success': False,
            'error': 'Error al llamar a Gemini: {}'.format(str(e))
        }
...
def analizar_modelo_glb(archivo_glb):
    """
    Analiza un archivo .glb usando Gemini API REST y retorna sugerencias sobre 
    daños estructurales y mejores materiales. Envía las texturas embebidas como imágenes.
    ...
    try:
        ...
        # Preparar información sin las imágenes base64 (para el JSON legible)
        info_para_prompt = {
            ...
            'texturas_info': [
                {
                    ...
                    'size_kb': round(t.get('size_bytes', 0) / 1024, 2) if t.get('size_bytes') else None
                }
        ...
        info_json = json.dumps(info_para_prompt, indent=2, ensure_ascii=False)
        
        # Agrupar elementos por tipo y textura para análisis detallado
        elementos_por_tipo = {
            'muros': [],
            'puertas': [],
            'ventanas': []
        ...
        resumen_elementos = {
            'muros': {
                ...
                'texturas_usadas': list(set([e['textura_idx'] for e in elementos_por_tipo['muros'] if e['textura_idx'] is not None])),
                'elementos': elementos_por_tipo['muros'][:3]  # Muestra primeros 3
            },
            'puertas': {
                'cantidad': len(elementos_por_tipo['puertas']),
                'texturas_usadas': list(set([e['textura_idx'] for e in elementos_por_tipo['puertas'] if e['textura_idx'] is not None])),
                'elementos': elementos_por_tipo['puertas'][:3]
            },
            'ventanas': {
                'cantidad': len(elementos_por_tipo['ventanas']),
                'texturas_usadas': list(set([e['textura_idx'] for e in elementos_por_tipo['ventanas'] if e['textura_idx'] is not None])),
...
""".format(
        ...
        if resultado.get('success'):
            # Limpiar respuesta de Gemini (remover markdown, espacios, etc)
        ...
        return {
            'success': False,
            'error': 'Error al analizar el modelo: {}'.format(str(e))
        }
